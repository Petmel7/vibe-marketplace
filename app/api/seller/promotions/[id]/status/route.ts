...
import { updateSellerPromotionStatus } from '@/features/promotions/promotions.service'
...
export async function PATCH(request: Request, { params }: Props): Promise<Response> {
  try {
    ...
    const data = await updateSellerPromotionStatus(user, id, body)
    ...
    return toErrorResponse('PATCH /api/seller/promotions/[id]/status', error)
  }
}
