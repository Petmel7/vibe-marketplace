...
import { rejectAdminRefundRequest } from '@/features/refunds/refunds.service'
...
export async function POST(request: Request, { params }: Props): Promise<Response> {
  try {
    ...
    const data = await rejectAdminRefundRequest(user, id, input)
    ...
    return toErrorResponse('POST /api/admin/refunds/[id]/reject', error)
  }
}
