...
import { getAdminStoreRiskProfileById } from '@/features/risk/risk.service'
...
export async function GET(_: Request, { params }: RouteContext): Promise<Response> {
  try {
    ...
    const data = await getAdminStoreRiskProfileById(user, id)
    ...
    return toErrorResponse('GET /api/admin/risk/stores/[id]', error)
  }
}
