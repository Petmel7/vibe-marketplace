 ...
 *
 * Returns all suspended sellers (admin only).
 *
...
export async function GET(request: NextRequest): Promise<Response> {
  try {
    ...
    const data = await getSuspendedSellers(user, parsed.data)
    return Response.json({ success: true, data })
  } catch (err) {
    return toErrorResponse('GET /api/admin/moderation/sellers/suspended', err)
  }
}
