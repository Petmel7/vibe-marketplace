...
export default async function AdminRiskUserDetailPage({
  ...
  const profile = await getAdminUserRiskDetailPageData(user, id)
  ...
  return (
    <AdminSection
      ...
      title="User risk detail"
      description="Inspect the full advisory signal history behind this account and use it to prioritize manual moderation review."
    >
      <div className="flex flex-wrap items-center gap-3">
        <Link href="/admin/risk/users" className="ui-link-muted">
          Back to user risk profiles
        </Link>
        <RiskRecalculateButton targetType="USER" targetId={id} label="Recalculate this user" />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-6">
          <RiskProfileSummaryCard profile={profile} entityType="USER" />
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className="mt-1 text-sm text-copy-secondary">
                  Review the signal history and open related reports or disputes for deeper context.
                </p>
              </div>

              <AdminFilterBar action={`/admin/risk/users/${id}`}>
                <StatusFilter
                  name="signalType"
                  label="Signal type"
        ...
        <aside className="space-y-6">
          <section className="ui-elevated-panel p-5 sm:p-6">
            ...
            <div className="mt-4 space-y-3 text-sm">
              <Link href={`/admin/users?search=${encodeURIComponent(profile.user?.email ?? '')}`} className="ui-link-muted">
                Open user account
              </Link>
