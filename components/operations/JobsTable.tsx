        ...
        {items.map((job) => (
          <tr key={job.id} className="border-t border-panelBorder align-top">
            <td className="px-5 py-4">
              ...
              <p className="mt-1 break-all text-xs text-copy-muted">{job.dedupeKey ?? 'No dedupe key'}</p>
            </td>
            <td className="px-5 py-4">
            ...
            <td className="px-5 py-4 text-copy-secondary">
              ...
              <p className="mt-1 text-xs text-copy-muted">Locked: {formatDateTime(job.lockedAt)}</p>
            </td>
            <td className="px-5 py-4 text-copy-secondary">
              {formatDateTime(job.runAt)}
              <p className="mt-1 text-xs text-copy-muted">Created: {formatDateTime(job.createdAt)}</p>
            </td>
